...
///
/// The 4-second animation only runs while [listening] or [thinking]; when idle
/// the controller is stopped and the orb renders one static frame, so it costs
/// nothing on the CPU/GPU while you're not talking to it. A [RepaintBoundary]
/// keeps unrelated rebuilds (e.g. the status line) from repainting the orb.
...
class _MicOrbState extends State<MicOrb> with SingleTickerProviderStateMixin {
  ...
  bool get _active => widget.listening || widget.thinking;

  @override
  void initState() {
    super.initState();
    if (_active) _c.repeat();
  }

  @override
  void didUpdateWidget(covariant MicOrb old) {
    super.didUpdateWidget(old);
    if (_active && !_c.isAnimating) {
      _c.repeat();
    } else if (!_active && _c.isAnimating) {
      _c.stop();
      _c.value = 0; // settle on a clean static frame
    }
  }
    ...
    return GestureDetector(
      ...
      child: RepaintBoundary(
        child: AnimatedBuilder(
          ...
          // The centred icon doesn't depend on `t`, so build it once and let
          // AnimatedBuilder pass it through rather than rebuild it each frame.
          child: SizedBox(
            width: widget.size,
            height: widget.size,
            child: Center(
